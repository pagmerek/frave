...
impl Frave {
    ...
    pub fn get_pixel(&self, x: u32, y: u32) -> u8 {
        let Pixel { r, g: _, b: _ } = self.image.get_pixel(x, y); // we assume grayscale for now
        r
    }

    #[inline]
    pub fn set_pixel(&mut self, x: u32, y: u32, v: u8) -> () {
        let gray: u8 = cmp::min(v, 255);
    ...
    pub fn find_coef(&mut self) {
        ...
        let rt: i32 = self.fn_cf((self.center + self.variant[(self.depth - 1) as usize]).clone(), 3, self.depth as i32 -2);
    ...
    fn fn_cf(&mut self, cn: Coord, ps: i32, dp: i32) -> i32 {
        let (mut lt, mut rt) = (0, 0); 
         if dp > 0 {
             ...
             rt = self.fn_cf(cn + self.variant[dp as usize], (ps<<1)+1, dp as i32 -1);
         } else {
            //dbg!(lt, rt, cn.x, cn.y);
             lt = self.get_pixel(cn.x as u32, cn.y as u32) as i32;
             rt = self.get_pixel((cn.x + self.variant[0].x) as u32, (cn.y + self.variant[0].y)as u32) as i32;
         }
    ...
    fn fn_vl(&mut self, sum: i32, ps: i32, cn: Coord, dp: i32) {
        ...
        if dp > 0 {
            ...
            self.set_pixel(cn.x as u32, cn.y as u32, lt as u8);
            self.set_pixel((cn.x + self.variant[0].x) as u32, (cn.y + self.variant[0].y) as u32, rt as u8);
        }
    }
}
